-- Keep the existing columns as Thai/default values and store optional English
-- names separately so current classroom references remain backwards compatible.
alter table public.classrooms
  add column if not exists name_en text,
  add column if not exists grade_level_en text;

-- Include bilingual classroom and subject names in assignment search results.
...
begin
  ...
  select
    ...
    jsonb_build_object(
      'id', s.id, 'code', s.code, 'name', s.name, 'name_en', s.name_en,
      'image_url', s.image_url, 'academic_year_id', s.academic_year_id,
      'semester_id', s.semester_id
    ),
    jsonb_build_object(
      'id', c.id, 'name', c.name, 'name_en', c.name_en,
      'grade_level', c.grade_level, 'grade_level_en', c.grade_level_en,
  ...
  where t.school_id = p_school_id
    ...
    and (
      ...
      or s.name_en ilike v_pattern escape '\'
      or c.name ilike v_pattern escape '\'
      or c.name_en ilike v_pattern escape '\'
      or c.grade_level ilike v_pattern escape '\'
      or c.grade_level_en ilike v_pattern escape '\'
