-- Keep teacher-assignment pagination server-side while allowing the list to
-- default to the school's active semester and browse historical semesters.
...
revoke all on function public.search_teacher_assignments(uuid, uuid, uuid, uuid, text, int, int)
  from public;
grant execute on function public.search_teacher_assignments(uuid, uuid, uuid, uuid, text, int, int)
  to service_role;
