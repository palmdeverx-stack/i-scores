...
import { canViewViaPermission, canManageViaPermission } from './department-permission-access';

// ----------------------------------------------------------------------

export const LESSON_PLAN_SELECT = `
  id, school_id, teacher_id, teacher_assignment_id, copied_from_id,
  title, unit_number, unit_name, duration_periods, start_date, end_date,
  learning_standards, indicators, learning_objectives, essential_content,
  ...
  teacher_assignment:teacher_assignments!lesson_plans_teacher_assignment_id_fkey(
    id, teacher_id,
    subject:subjects(id, code, name, learning_standards, indicators, learning_units),
