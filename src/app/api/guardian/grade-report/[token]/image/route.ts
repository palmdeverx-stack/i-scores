...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  try {
    const image = await renderStudentGradeReportImage(report);
    await supabaseAdmin
      .from('grade_report_share_tokens')
      .update({
    ...
    return NextResponse.json({ message: 'ไม่สามารถสร้างรูปภาพผลการเรียนได้' }, { status: 500 });
  }
}
